...
def _get_api_key() -> str:
    try:
        ...
        raise RuntimeError(f"Could not load API key from config: {e}")
...
def _run_generated_code(description: str, speak: Callable | None = None) -> str:
    ...
    if not desktop.exists():
        try:
            ...
            # winreg not available or key not found - use default
            print(f"[Executor] Warning: Could not load Desktop path from registry: {e}")

    client = genai.Client(api_key=_get_api_key())
    model = client.models.get_model(model="gemini-2.5-flash")
        ...
        try:
            ...
            if tmp_path:
                try:
                    ...
                    print(f"[Executor] Warning: Could not delete temp file {tmp_path}: {e}")
...
def _detect_language(text: str) -> str:
    from google import genai
    client = genai.Client(api_key=_get_api_key())
    model = client.models.get_model(model="gemini-2.5-flash-lite")
    try:
        response = model.generate_content(
...
def _call_tool(tool: str, parameters: dict, speak: Callable | None) -> str:
    ...
    if tool == "open_app":
        ...
        raise ValueError(
            f"Unknown tool '{tool}'. Use only tools defined in the planner catalog."
        )
